...
export function registerDomainCommand(program: Command): void {
  const domain = program.command('domain').description('Manage domain context files');
  ...
  domain
    ...
    .action(
      async (
        ...
        const overviewUpdated = updateOverviewDomainIndex(
          overviewPath,
          domainName,
          options.description,
        );
