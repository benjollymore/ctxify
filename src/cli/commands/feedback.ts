...
export function registerFeedbackCommand(program: Command): void {
  program
    ...
    .action(
      async (repo: string, options: { body: string; type: string; source?: string; dir?: string }) => {
      ...
      if (entryType !== 'correction' && entryType !== 'antipattern') {
        console.log(
          JSON.stringify({ error: `Invalid --type "${entryType}". Must be "correction" or "antipattern".` }),
        );
        process.exit(1);
      }
      ...
      if (entryType === 'antipattern') {
        ...
        const entry = formatAntiPatternEntry({ body: options.body, source: options.source, timestamp });
